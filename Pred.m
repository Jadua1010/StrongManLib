...
while true
    ...
    gate1Status = ~ioPinData(1);        % Invert data
    gate2Status = ~ioPinData(2);        % Invert data
...
disp(['Start Time (t1): ', char(t1)]);
disp(['End Time (t2): ', char(t2)]);
...
% Display x(t) values
disp('x(t) values:');
disp(table(t', x_t', 'VariableNames', {'t', 'x_t'}));

% Plot the function
plot(t, x_t, 'b-', 'LineWidth', 2);
xlabel('Time t');
ylabel('x(t)');
title('Plot of x(t) between 0 and 1 with Determined Constants C1 and C2');
grid on;
